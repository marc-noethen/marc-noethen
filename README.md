<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:0077b6,100:00b4d8&height=180&section=header&text=Marc%20Nöthen&fontSize=42&fontColor=ffffff&fontAlignY=38&desc=Cyber%20Security%20Analyst%20·%20Python%20Developer%20·%20Learning%20in%20Public&descSize=15&descAlignY=58&descColor=90e0ef" width="100%"/>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=20&pause=1200&color=00B4D8&center=true&vCenter=true&width=650&lines=Fachinformatiker+AE+%E2%9C%85+%7C+Security+Spezialisierung+%F0%9F%94%90;Vollzeit-Ausbildung+%40+Cybersteps+%E2%80%94+Modul+3;Python+%7C+Automation+%7C+Log+Analysis+%7C+DFIR;Ich+lerne+%C3%B6ffentlich+%E2%80%94+jeden+Tag." alt="Typing SVG" />
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/marc-j-nöthen-b201a315b/">
    <img src="https://img.shields.io/badge/LinkedIn-Marc%20Nöthen-0A66C2?style=flat&logo=linkedin&logoColor=white"/>
  </a>
  &nbsp;
  <a href="https://github.com/marc-noethen/cybersteps-edu-paths">
    <img src="https://img.shields.io/badge/Lernjournal-cybersteps--edu--paths-00b4d8?style=flat&logo=github&logoColor=white"/>
  </a>
  &nbsp;
  <img src="https://img.shields.io/badge/Standort-Leipzig%2C%20DE-555?style=flat&logo=googlemaps&logoColor=white"/>
</p>

---

## Über mich

Ausgebildeter **Fachinformatiker Anwendungsentwicklung (IHK)**, jetzt in der Vollzeit-Spezialisierung zum **Cyber Security Analyst & Consultant** bei [Cybersteps](https://cybersteps.de) — aktuell Modul 3.

Ich lerne mit System: Obsidian als Wissensdatenbank, die Birkenbihl-Methode kombiniert mit KI-generierten Audio-Zusammenfassungen, und einem eigenen "Brain-Extraction"-Prozess der Theorie in anwendbares Wissen überführt. Parallel dazu: 30+ Fachbücher, Python Projekte, Full Stack Open, und eigene Python-Tools.

> "Es gibt nur zwei Arten von Unternehmen: Diejenigen, die gehackt wurden, und diejenigen, die noch nicht wissen, dass sie gehackt wurden." – John Chambers

---

## 🎓 Bildungsweg

| Institution | Inhalt | Status |
|---|---|---|
| [Cybersteps](https://cybersteps.de) | Cyber Security Analyst & Consultant — Modul 3 | 🔄 Aktiv seit 04/2026 |
| IHK | Fachinformatiker Anwendungsentwicklung | ✅ 05/2025 |
| educx GmbH | Python — 180 Std. (OOP, Pandas, NumPy, EDA, Scraping) | ✅ 05 /2025 |
| Udemy / Dr. Angela Yu | The Complete 2024 Web Development Bootcamp — 61.5 Std. | ✅ 10/2024 |
| Udemy | SQL Grundlagen Bootcamp — 9 Std. | ✅ 05/2024 |
| SoloLearn | PHP Course | ✅ 07/2022 |
| Udemy / Jan Schaffranek | Fortgeschrittene Python Programmierung — 19 Std. | ✅ 10/2021 |
| Udemy / Eric Amberg | CompTIA Network+ — 24.5 Std. | ✅ 09/2021 |
| Udemy | Python Grundlagen Bootcamp — 24.5 Std. | ✅ 05/2021 |

---

## 🔐 Cybersteps — Was ich bisher gelernt habe

> Vollständige Notizen, Übungen & Zusammenfassungen: [`cybersteps-edu-paths`](https://github.com/marc-noethen/cybersteps-edu-paths)

| Modul | Themen |
|---|---|
| **Modul 1** | Netzwerke, Linux, Python, Web App Security, Cloud Security, Kryptographie |
| **Modul 2** | Malware Analysis (statisch & dynamisch), Malware Defense, SIEM, Network & Endpoint Monitoring, DFIR (Disk, Memory, Network Forensics) |
| **Modul 3** | 🔄 Läuft seit 13.04.2025 |

**Lernmethode pro Woche:**
- Theorie → Zusammenfassung (Birkenbihl-Methode)
- KI-Audio-Dateien zur Wiederholung
- Übungen + eigene Lösungen
- Wochenprojekt zur Anwendung

---

## 🚀 Projekte

| Projekt | Beschreibung | Stack |
|---|---|---|
| [InsightLog\_MJN](https://github.com/marc-noethen/InsightLog_MJN) | Python-Tool zum Parsen und Analysieren von Logs — erkennt verdächtige Muster und macht Rohdaten lesbar | Python |
| [file\_mover](https://github.com/marc-noethen) | Automatisierungstool: verschiebt rekursiv Dateien aus Unterordnern, mit Duplikat-Handling | Python |
| [Ordner-ASCII-Tree](https://github.com/marc-noethen) | Generiert ASCII-Baumstruktur eines Verzeichnisses als Textdatei | Python |
| [taskflow](https://github.com/marc-noethen) | Smart Task Manager — aus dem Python Mega Course | Python |
| [Full Stack Open](https://github.com/marc-noethen) | Kurs der Uni Helsinki: React, Node.js, GraphQL, TypeScript, Docker, CI/CD | JS / TS |
| [Networking Projekt](https://github.com/PabloPAF/NetworkigP1_B_Wazzap) | Chat-App als Netzwerkprojekt (Kollaboration) | Python |

---

## 📚 Was ich gerade lese

**Security & Hacking**
`Linux Basics for Hackers` · `Black Hat Python` · `Hacking & Security (Kofler)` · `Gray Hat Hacking` · `Hacker Playbook 1–3` · `Practical Malware Analysis` · `Blue Team Handbook`

**Cloud & Forensics**
`Practical Cloud Security` · `Digital Forensics & Incident Response` · `Practical Threat Detection Engineering`

**AI & Engineering**
`Designing ML Systems (Chip Huyen)` · `AI Engineering` · `The Pragmatic Programmer` · `Clean Coder` · `Refactoring (Fowler)`

> Vollständige Leseliste mit Notizen: [`reading-notes`](https://github.com/marc-noethen) *(coming soon)*

---

## 🛠️ Tech Stack

**Security & Analyse**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat&logo=gnubash&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Wireshark](https://img.shields.io/badge/Wireshark-1679A7?style=flat&logo=wireshark&logoColor=white)
![CompTIA](https://img.shields.io/badge/CompTIA_Network+-C8202A?style=flat&logo=comptia&logoColor=white)

**Daten**

![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)

**Web & Dev**

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat&logo=php&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

---

## 📊 GitHub Stats

<p align="center">
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=marc-noethen&show_icons=true&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=00b4d8&icon_color=00b4d8&text_color=cdd6f4"/>
  <img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=marc-noethen&layout=compact&theme=github_dark&hide_border=true&bg_color=0d1117&title_color=00b4d8&text_color=cdd6f4"/>
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=marc-noethen&theme=github-dark-blue&hide_border=true&background=0d1117&ring=00b4d8&fire=00b4d8&currStreakLabel=00b4d8" width="480"/>
</p>

---

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00b4d8,50:0077b6,100:0d1117&height=100&section=footer" width="100%"/>
